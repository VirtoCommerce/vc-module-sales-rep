...
* **Case-insensitive matching by DB collation, not code.** SqlServer and MySql default collations are already case-insensitive; on PostgreSQL the migration creates the platform's `case_insensitive` ICU collation and applies it to the metadata `Name` and `Category` columns. Category values are **not normalized** — each document keeps the casing it was saved with; values differing only by case count as one category for filtering and counting, and the category listing shows one representative casing of the group (typically the first created). ⚠️ **PostgreSQL 17+ is required** for the documents keyword search — older versions reject `LIKE` on nondeterministic collations.
