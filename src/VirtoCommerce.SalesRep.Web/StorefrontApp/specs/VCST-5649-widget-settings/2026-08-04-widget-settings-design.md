...
has been going out as `[]`. Only the two `.graphql` documents change, to _select_ `settings` back.
...
- `SalesRepLayoutSetting { key: String!, value: AnyValue }`, introspected anonymously against
  ...
  so `yarn generate:graphql-types` runs against that env; the _data_ fields do need a rep token.
...
Writing only the _unchecked_ rules is what carries AC #5 across the tab half: a status the backend
