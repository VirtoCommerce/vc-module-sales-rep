...
per page — the customer-profile layout applies to _every_ customer, not per-customer.

## At a glance (as built)

The sections below were written _before_ implementation and a few names drifted (the registry landed
...
**The whole feature in one equation.** The registry owns _structure_, the saved document owns only
_arrangement_:
...
1. **Engine against existing blocks.** Build the layout mechanism over what the repo has today; the
   ...
   _not_ built here. Future widgets plug into the registry for free.
2. **The KPI stat row participates — reorder _and_ hide.** Three regions total.
...
5. **DnD library: `sortablejs`.** _As built:_ used directly, not through
...
Net effect: the persisted document is only ever _order + hidden_.
...
- **Mutation fails:** stay in edit mode, surface the error, keep the draft. The rep's work is never
  silently discarded. _As built:_ focus returns to Save as well — starting the save makes the wrapper
...
- **Query errors** (as distinct from returning `null`): render registry defaults but **disable Edit
  ...
  how a layout gets destroyed. `null` is the normal never-saved case and does _not_ disable editing.
  _As built:_ `loadFailed` drives an alert on both pages — silently swapping in defaults with no edit
...
- **Navigating away mid-edit: nothing is guarded, deliberately.** _Decided 2026-07-29:_ an
...
_As built:_ used directly rather than through `useSortable` from `@vueuse/integrations`. That wrapper's
...
**`onUpdate` alone does not cover cross-list drags.** It fires only for a reorder _within_ one list;
...
_As built:_ `layout-region.vue` handles this in a single `onEnd` rather than the `onAdd`/`onRemove`
...
The case that makes `restore()` load-bearing is a drop whose state change is _refused_ (a save in
...
- **The SortableJS placeholder.** SortableJS adds `.sortable-ghost` (the dragged element, which it
  ...
  _Corrected 2026-07-29:_ this originally read the prototype's `.45` fade as a workaround for native
...
- **Dark mode — no override needed, but "every token" was doing the work.** _Corrected 2026-07-29:_
